classdef rombach_config 
	...
	methods (Access = private)
		...
		function [C,P,Q,score] = simulated_annealing(self,A,alpha,beta)
			...
			aopt.Generator = @gen;
			aopt.Verbosity = 0;
			deg = sum(A,2);m= sum(deg)/2;
			[x,fval] = anneal(@(x)(-self.eval(A,deg,m,x,alpha,beta)),x,aopt);
...
%		end	
		...
		function q = eval(self,A,deg,m,x,alpha,beta)
			c = self.corevector(x,alpha,beta);	
			q = c'*A*c - c'*(deg*deg'/(2*m))*c; 
			%q = c'*A*c - sum(c'*deg)^2/(2*m); 
		end
		...
		function model = init_model(self,model)
			...
			if ~isfield(model,'alpha');model.alpha = 0.5;end
