...
classdef cucuringu_cp < cpabst
	...
	methods ( Access = private )	
		...
		function [C,P,Q,Qs,score] = lapsgn_core(self, G, param)
			A = G.adjacency_matrix();
			deg = sum(A,2);N = size(A,1);
			denom = 1./deg;denom(isinf(denom)) = 0;denom(isnan(denom)) = 0;
			T = sparse(1:N,1:N,denom) * A;
			N = size(T,1);
			if N <1000	
				[v,d] = eig(full(T));
				[val,ord] = sort(diag(d));
			...
			score = v;
			if (q1 > Q) | (abs(q1-Q) < eps*1000 & rand()>0.5)
				score = -score;
				C = 1-C;Q = q1; 
			end
			Qs = Q;
		...
		function [C,P,Q,Qs,score] = lap_core(self, G, param)
			...
			D = sum([C,P]' * G.adjacency_matrix(),2);
			N = sum([C,P],1)
			if D(1)/N(1) < D(2)/N(2)
				C = 1-C;
				P = 1-P;
