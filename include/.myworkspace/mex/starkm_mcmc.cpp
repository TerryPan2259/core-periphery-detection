...
void starkm_mcmc(int maxStableLoopNum,int maxItNum){
	// --------------------------------
	// Initialise
	// --------------------------------
	...
	while((itnum - lastupdate)<=maxStableLoopNum & itnum <=maxItNum){
		...
		// propose an assignment of nodes 
		C[nid] = !C[nid];
		findPeriphery();
		double Qprop = calcQ();
			
		itnum++;	
		if( udist(mtrnd) >= exp( beta * (Qprop-Q)) ) {
			C[nid] = !C[nid];
			continue;
		}
...
{ 
   	...
   	beta = (double) mxGetPr(prhs[5])[0]; // maximum number of intervals for updating Qmax 
	for(int i = 0;i < N;i++){
		std::vector<int> tmp;
	...
	starkm_mcmc(maxStableLoopNum,maxItNum);
